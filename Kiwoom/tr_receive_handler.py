...
if TYPE_CHECKING:
    from Kiwoom.KiwoomAPI import API


def on_receive_opt10080(kw: 'API', sRQName, sTrCode):
    """주식분봉차트조회요청 완료 후 서버에서 보내준 데이터를 받는 메소드"""

    data_cnt = kw.get_repeat_cnt(sTrCode, sRQName)
    ohlcv = {'date': [], 'open': [], 'high': [], 'low': [], 'close': [], 'volume': []}

    for i in range(data_cnt):
        tradetime = kw.get_comm_data(sTrCode, sRQName, i, "체결시간")
        open = kw.get_comm_data(sTrCode, sRQName, i, "시가")
        high = kw.get_comm_data(sTrCode, sRQName, i, "고가")
        low = kw.get_comm_data(sTrCode, sRQName, i, "저가")
        close = kw.get_comm_data(sTrCode, sRQName, i, "현재가")
        volume = kw.get_comm_data(sTrCode, sRQName, i, "거래량")

        ohlcv['tradetime'].append(tradetime)
