    ...
    def run(self):
        while True:
            ...
            for code in df_copy:
                ...
                self.signal.portfolio_stock_dict[code].update({"average": m_algo.minute_df['average'][-2]}) # D-1값
                self.signal.portfolio_stock_dict[code].update({"MA10": m_algo.minute_df['MA10'][-2]}) # D-1값
                QTest.qWait(1000)
            time.sleep(90)
